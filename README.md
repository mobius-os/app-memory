...
Analysts cite short, closed-set source handles; trusted runner code expands them to canonical chat IDs before applying the same strict provenance check. A semantically invalid primary result is recorded with a safe rejection code and does not suppress a configured fallback. The runner alternates focused chat and recall-audit work items until its actual schedule window approaches, then publishes once. Work observed by a failed run remains pending until a successful analyst run actually receives it, so recovery does not depend on that source remaining in tomorrow's latest-results window.
