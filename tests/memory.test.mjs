...
test('reader returns a bounded catalogue then verified pinned body pages', () => {
  ...
  assert.match(reader, /recall_execution\(request\.lookup_id\)/)
  assert.match(expander, /load_recall_manifest\(lookup_id\)/)
...
test('settings expose app-level background agent overrides', () => {
  ...
  assert.match(source, /primaryAgentMode === 'app' \? 'app' : 'system'/)
  assert.match(source, /secondaryAgentMode === 'app' \? 'app' : 'system'/)
  assert.match(source, /Background agents/)
  assert.match(source, /<ModelPicker/)
  assert.match(source, /useSettingsDefault=\{primaryAgentMode === 'system'\}/)
  assert.match(source, /useSettingsDefault=\{secondaryAgentMode === 'system'\}/)
  assert.match(source, /onSettingsDefault=\{\(\) => setPrimaryAgentModeChoice\('system'\)\}/)
